...
def parse_rss_date_to_ist(entry):
    ...
    Edge cases handled:
    - Future-dated articles (age_hours < 0): clamped to 0.0 so they pass the 24hr filter
    - Missing timestamp: falls back to current time with age_hours=0.5 (passes 24hr filter)
    """
    ...
    return dt_ist.strftime("%b %d, %Y @ %I:%M %p IST"), 0.5, current_ts
